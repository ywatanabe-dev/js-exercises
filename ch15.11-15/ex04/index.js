...
document.addEventListener("DOMContentLoaded", () => {
  try {
    ...
    if (toDoTasks !== null) {
      ...
      localStorage.toDoTasks = JSON.stringify([]);
    }
  } catch {
...
form.addEventListener("submit", (e) => {
  ...
  try {
    const tasks = JSON.parse(localStorage.toDoTasks);
    tasks.push(task);
...
function appendToDoItem(task) {
  ...
  destroy.addEventListener("click", () => {
    try {
      const prevTasks = JSON.parse(localStorage.toDoTasks);
      const newTasks = prevTasks.filter((item) => item.id !== task.id);
      localStorage.toDoTasks = JSON.stringify(newTasks);
    } catch {
      console.log("can not use localStorage.");
    }
